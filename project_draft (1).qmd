...
After exploring and analyzing our data, we propose to fit the data with multiple linear regression. Our focus will be on the distance of the race, competition type, gender of the participant, and proximity to the meteorological station categorical variables, as well as the air temperature, heat index, wind speed, relative humidity, and dew point continuous predictors. We will also consider interaction terms between both categorical and continuous predictors; however, as our goal is to describe the relationship between weather and elite athlete performance rather than make predictions, we will aim to keep our model simple for interpretation.

Our preliminary model selection will be guided by the `stepAIC()` function with BIC to narrow down to a few potential combinations of predictors. We will then complete further analysis by completing significance tests for individual predictors, reviewing residuals for model fit, and identifying any influential points.

## Methods and Model Building

We start by fitting a simple model with all of our previously interested parameters and perform a step-wise model selection. The resulting summary shows that air_temp is omitted, which aligns with our previous analysis.However, we will look into VIF to see if there's still evidence multicollinearity.

```{r}
## fitting full model
full_model <- lm(percent_standing_1st ~ competition + distance + air_temp + heat_index + wind_speed +rel_hum + dew_point + sex + proximity + simp_wet_bulb, data = results)
...
We observe high GVIF values (same as VIF values for continuous variable) for heat_index, rel_hum, simp_wet and dew_point as well as competition and distance (categorical variables). However, we have to consider the degree of freedom and the corresponding GVIF\^(1/(2\*Df)) values of competition and distance, which are less than common acceptance rule. Given the significance of each variable in the model and the VIF results, we consider removing simp_wet_bulb and maybe dew_point due to its high VIF, especially since heat_index already accounts for temperature and humidity effects, and rel_hum provides additional unique information about humidity.

```{r fig.height = 4, fig.width=4}
## avp
par(mfrow=c(2,2))
avPlot(step_model, "heat_index")
avPlot(step_model, "simp_wet_bulb")
avPlot(step_model, "dew_point")
avPlot(step_model, "rel_hum")

```

(the full model with dew_point still has higher r-squared so it really come down to if we prioritize model explanatory power or multicollinearity. Here, i removed it but we can still keep it and remove heat_index, i feel like it is justifiable via avp plots. Also the gvif\^.. is for categoricals var or high order variables and we can treat that like normal vif https://stats.stackexchange.com/questions/70679/which-variance-inflation-factor-should-i-be-using-textgvif-or-textgvif)

```{r}
## reduced model
reduced_model <- lm(percent_standing_1st ~ competition + distance + heat_index + wind_speed +rel_hum + sex + proximity , data = results)
summary(reduced_model)
vif(reduced_model)
```

Looking at the new vif values, we see no evidence of multicollinearity. Now that the model is more stable, we will add in interested interaction terms mentioned before. (distance:heat_index, sex:heat_index, and rel_hum:sex)

```{r}
## new model
new_model <- lm(percent_standing_1st ~ competition + wind_speed + proximity + distance * heat_index + (heat_index + rel_hum) * sex, data = results)

## multicollinearity check
vif(new_model, type = 'predictor') 
...
We again check for multicollinearity and observe all GVIF\^(1/(2\*Df)) values to be within the acceptable range. (We can do some interpretation on the model here with respect to the baseline categories).
...
The standardized residuals vs leverage plot show no influential points with respect to Cook's distance, but we do observe some possible outliers (obs 203, 205, and 623). Other than that, the constant variance assumption seems fine but the normal QQ plot poses violation regarding the normality assumption (the tails are heavy and deviate from the line).

(Since rel_hum:sexWomen and proximity are insignificant, we can remove them.)

We might try to perform a box-cox transformation to resolve this issue. Since boxcox only works when dependent variable is strictly positive, we will use the "tweak" version of that, Yeo-Johnson transformation. https://statisticaloddsandends.wordpress.com/2021/02/19/the-box-cox-and-yeo-johnson-transformations-for-continuous-variables/

```{r fig.height = 4}
## bc
bc <- boxCox(new_model, family="yjPower", plotit = TRUE) ## doesnt include 1

## lambda value
lambda <- bc$x[which.max(bc$y)]

## transformation
depvar_transformed <- yjPower(results$percent_standing_1st, lambda)

## fit model
transformed_model <- lm(depvar_transformed ~ competition + wind_speed + proximity + distance * heat_index + (heat_index + rel_hum) * sex, data = results)
...
## not much has changed with the normal qq since lambda is approx 0.90909
```

```{r}
## independent var transformation
boxTidwell_results <- boxTidwell(percent_standing_1st ~  + heat_index + rel_hum, 
                                  ~ competition + proximity + distance + sex +  wind_speed , 
                                  data = results)
boxTidwell_results

# Apply the Box-Tidwell power transformation
results$heat_index_transformed <- (results$heat_index^4 - 1) / 4
results$rel_hum_transformed <- (results$rel_hum^22 - 1) / 22

transformed_model1 <- lm(percent_standing_1st ~ competition + wind_speed + proximity + distance * heat_index_transformed + (heat_index_transformed + rel_hum_transformed) * sex, data = results)

summary(transformed_model1)
plot(transformed_model1)
## this is a bit better but the interpretation will be harder lol
```
