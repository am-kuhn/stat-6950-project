...
knitr::opts_chunk$set(fig.height = 2, fig.width = 6,
                      echo = FALSE, fig.align = 'center', fig.pos = 'H',
...
# Review response variable and create histogram.
#summary(results$percent_standing_avg)
...
weather = c("air_temp", "dew_point", "wind_speed", 
            "adj_wind_speed", "rel_hum", "clouds_OKTA",
            "solar_rad", "heat_index", "simp_wet_bulb", 
            "wet_bulb")
...
```{r include = FALSE}
# Plots used for review but not included in report.
...
cat.counts = as.data.frame(table(results$distance, 
                                 results$competition, 
                                 results$sex))
cat.counts = cat.counts %>% 
  pivot_wider(names_from = c("Var1", "Var3"), values_from = "Freq")
cat.counts$Var2 = c("Common Wealth", "Diamond League", "Gold Marathon",
                    "Olympic", "World Champ.", "World Cup")

table = cat.counts %>% 
  kable(col.names = c("Comp.", "10K", "20K", "3K", "50K", "5K", "Mar.",
                      "10K", "20K", "3K", "50K", "5K", "Mar."), 
        booktabs = FALSE) %>% 
  add_header_above(c(" ", "Men" = 6, "Women" = 6)) %>% 
  kable_styling(latex_options = c("striped", "HOLD_position"), full_width = FALSE)
table
```
Now, assessing the relationships between our categorical variables and our response variable, we see that the percent deviation from the event record does not appear to show significant variability across competition type, although some variability is present (@fig-comp-boxplot).
```{r fig.height = 2}
#| label: fig-comp-boxplot
#| fig-cap: "Percent Deviation by Competition Type."

# Plot of percent deviation by competition.
ggplot(data = results, aes()) + 
...
# Plots of percent deviation and other predictors.
...
```
To demonstrate the possibility of an interaction term between a weather predictor and a categorical variable, @fig-distance-plots(b) shows a relationship between heat index and distance. It is possible that the slope of a regression line for `percent_standing_avg` on `heat_index` will differ for each level of `distance`. Further, we see the slight curvature recognized in the earlier scatterplot matrix. 
...
We start by fitting a simple model with all of our previously noted potential parameters and perform step-wise, forward, and backward model selection using AIC. We may express our main effects full model as:
\begin{align*}
Y_i &= \beta_0 + \beta_{11}U_{11i} \\
& \quad \quad \quad + \beta_{21}U_{21i}+ \beta_{22}U_{22i}+ \beta_{23}U_{23i}+ \beta_{24}U_{24i}+ \beta_{25}U_{25i} \\
& \quad \quad \quad + \beta_{31}U_{31i}+ \beta_{32}U_{32i}+ \beta_{33}U_{33i}+ \beta_{34}U_{34i}+ \beta_{35}U_{35i}+ \\
& \quad \quad \quad + \beta_1X_{1i}+\beta_2X_{2i}+\beta_3X_{3i}+\beta_4X_{4i}+\beta_4X_{5i}+\beta_4X_{6i} \\
& \quad \quad \quad +\beta_{41}U_{41i} +e_i
\end{align*}
where we have $e_i \overset{iid}{\sim} N(0,\sigma^2), \ i=1,...,650$ and define the parameters as such:  

\begingroup
\leftskip4em
\rightskip\leftskip
$Y_i$ is the percent deviation from standing record of race $i$ versus the average top 3 finishing times.  
$U_{11i}$ = 1 if its a female race and 0 otherwise.  
$U_{21i}$ = 1 if its a Diamond League competition.  
$U_{22i}$ = 1 if its a Gold Marathon competition.  
$U_{23i}$ = 1 if its a Olympic competition.  
$U_{24i}$ = 1 if its a World Championships competition.  
$U_{25i}$ = 1 if its a World Cup competition.  
$U_{31i}$ = 1 if its a 20k distance race.  
$U_{32i}$ = 1 if its a 3k distance race.  
$U_{33i}$ = 1 if its a 50k distance race.  
$U_{34i}$ = 1 if its a 5k distance race.  
$U_{35i}$ = 1 if its a Marathon distance race.  
$U_{41i}$ = 1 if the proximity of the race to the closest weather station was far (> 10 miles).  
$X_{1i}, X_{2i}, X_{3i}, X_{4i}, X_{5i}, X_{6i}$ is the dew point, relative humidity, wind speed, heat index, air temperature, and simplified WBGT of $i$th race.
\par
\endgroup

Each selection process returned similar predictors with the only variation coming from the forward selection model which excluded `dew_point`,  `rel_hum`, `simp_wet_bulb` and `proximity` as predictors while the step-wise and backward selection models excluded `air_temp` and `simp_wet_bulb`. Therefore, we will begin our analysis with the step-wise/backward selection model and consider more stringent parameter selection criteria due to multi-collinearity in predictors. Additionally, we will explore adding interaction terms to determine whether relationships exist between weather predictors and our categorical variables. 

```{r include = FALSE}
# Step-wise, forward, and backward searches using AIC.
...
stepMod <- stepAIC(object = fullMod, 
                   direction = "both", 
                   k = 2, trace = FALSE)
forwardMod <- stepAIC(object = nullMod, 
                      scope = list(upper = fullMod), 
                      direction = "forward", 
                      k = 2, trace = FALSE)
backwardMod <- stepAIC(object = fullMod, 
                       direction = "backward", 
                       k = 2, trace = FALSE)
# summary(stepMod)
# summary(forwardMod)
...
Method  | Sex | Comp. | Dist. | Air Temp. | Dew Point | Rel. Hum. | Wind | Heat Index | sWBGT | Prox.
--------|-----|-------|-------|--------|--------|--------|------|--------|-------|---------
`SW` | \mbox{\color{red} $\bullet$} | \mbox{\color{red} $\bullet$} | \mbox{\color{red} $\bullet$} | | \mbox{\color{red} $\bullet$} |  \mbox{\color{red} $\bullet$} | \mbox{\color{red} $\bullet$} | \mbox{\color{red} $\bullet$} | | \mbox{\color{red} $\bullet$} |
`FS` | \mbox{\color{red} $\bullet$} | \mbox{\color{red} $\bullet$} | \mbox{\color{red} $\bullet$} | \mbox{\color{red} $\bullet$} | | | \mbox{\color{red} $\bullet$} | \mbox{\color{red} $\bullet$} | | |
`BE` | \mbox{\color{red} $\bullet$} | \mbox{\color{red} $\bullet$} | \mbox{\color{red} $\bullet$} | | \mbox{\color{red} $\bullet$} | \mbox{\color{red} $\bullet$} | \mbox{\color{red} $\bullet$} | \mbox{\color{red} $\bullet$} | | \mbox{\color{red} $\bullet$} |

To assess potential multi-collinearity, we will look at the VIF (GVIF) values for our step-wise model (@tbl-vif-stepwise). As expected, we see that dew point, relative humidity, and heat index show high collinearity. Therefore, given the significance of each predictor and our previously observed correlations, we will remove dew point from the model. Air temperature showed strong collinearity with heat index, which we desire to maintain in the model for inference and interpretation purposes.
```{r}
#| label: tbl-vif-stepwise
#| tbl-cap: "VIF for Step-Wise Model"

## VIF for step-wise model.
kable(vif(stepMod), digits = 4)%>% 
  kable_styling(latex_options = "striped", full_width = FALSE)


```

After refitting our model, our VIF values do not suggest evidence of multi-collinearity. However, to explore whether relative humidity or wind speed explain any additional variability in the percent deviation from event record after accounting for all other parameters in the model, we may review added variable plots for `rel_hum` and `wind_speed`. @fig-avp(a) and @fig-avp(b) both appear to be null plots, which suggests that relative humidity and wind speed do not explain any additional variability. This is supported by the results of ANOVA tests on each parameter at a family-wise error rate of 5\%. Further, the ANOVA tests (@tbl-anova-reduced) suggest that distance and proximity are also not significant. We will remove proximity as a predictor, as we only hoped to explore whether there was a relationship between distance to meteorological station and percent deviation, but it is possible that the relationship only lies in the integrity of the weather data itself. We will maintain distance in the model as we hope to consider interaction terms with this parameter.
```{r fig.height = 4}
#| label: fig-avp
#| layout-ncol: 2
#| fig-cap: "Added Variable Plots"
#| fig-subcap: 
#|    - "Relative Humidity AVP"
#|    - "Wind Speed AVP"

# Fit step-wise model without noted collinear points.
reducedMod <- lm(percent_standing_avg ~ sex + competition + distance + 
                   wind_speed + heat_index + rel_hum + proximity, data = results)
# Added variable plots for reduced model.
...
```{r}
#| label: tbl-anova-reduced
#| tbl-cap: "ANOVA Test for Reduced Model"

# ANOVA and VIF for reduced model.
...
```
As we hope to consider whether the heat index effects different race distances differently, we will add an interaction term between the continuous predictor `heat_index` and the categorical predictor `distance`. From the ANOVA tests at a family-wise error rate of 5\%, we see that the new interaction term is considered significant (@tbl-anova-int). While the main effect `distance` is not significant, we will keep it in the model to maintain the principal of marginality. 
```{r}
#| label: tbl-anova-int
#| tbl-cap: "ANOVA Test for Interaction Model"

# Fit model with interaction term.
intMod <- lm(percent_standing_avg ~ sex + competition + distance + 
               heat_index + distance:heat_index, data = results)
...
```{r include = FALSE}
# Consider Box-Cox transformation using Yeo-Johnson family 
# to allow for negative response values.
...
#|    - "Studentized Residuals (Outlier)"


# Residual diagnostics for initial interaction model.
...
base + geom_point(aes(x = as.numeric(case), y = .ti)) + 
  ...
  geom_hline(yintercept = qt(1 - (0.05/(2*dim(results)[1])), df = 632 - 1), 
             linetype = "dotted") +
  geom_hline(yintercept = qt((0.05/(2*dim(results)[1])), df = 632 - 1), 
             linetype = "dotted") + 
  ylim(-5,5) +
...
# Make temporary dataframe to find the cases with the extreme 
# Cook's distance and studentized residuals.
outlierTestDF = augment(intMod, results) %>% 
  mutate(.ti = rstudent(intMod)) %>% 
  rownames_to_column("case")
# Sort studentized residuals. Cases 590, 589, and 177 are our identified outliers.
head(outlierTestDF[order(outlierTestDF$.ti, decreasing = TRUE), 
                   c(".ti", "case")])
head(outlierTestDF[order(outlierTestDF$.ti, decreasing = FALSE), 
                   c(".ti", "case")])
# Sort Cook's distances. Case 171 is our identified influencer.
head(outlierTestDF[order(outlierTestDF$.cooksd, decreasing = TRUE), 
                   c(".cooksd", "case")])
...
intMod.new <- lm(percent_standing_avg ~ sex + competition + distance + 
                   heat_index  + distance:heat_index, data = results.new)
anova(intMod.new)
```

After refitting our model without the influential cases, we reviewed the updated model and residual diagnostics (see @fig-model-diagnostics-outlier and @fig-residual-diagnostics-outlier). The plots did not suggest any significant changes in model fit. After observing the slight curvature in the relationship between heat index and percent deviation, we fit a model including the squared heat index as an additional term. While this parameter did improve model fit, we did not believe the improvement outweighed the cost in interpretability. We also acknowledge that the quadratic relationship appears to be more pronounced for the 20K and 50K race-walks and marathons, as evident in @fig-distance-plots(b). Given more time to complete our data set, we would have liked to have expanded our model to individual distances and improve our predictive performance. 

```{r include = FALSE}
# Attempted polynomial approach.
testMod <- lm(percent_standing_avg ~ sex + competition + distance + heat_index, 
              data = results.new)
...
Our final selected model includes sex, competition, distance, heat index, and the interaction between distance and heat index to predict the percent deviation between the top three average finishing times and the standing event record. We may express our final linear model as:
\begin{align*}
Y_i & = \beta_0 + \beta_{11}U_{11i} + \beta_{21}U_{21i}+ \beta_{22}U_{22i}+ \beta_{23}U_{23i}+ \beta_{24}U_{24i}+ \beta_{25}U_{25i} \\
& \quad \quad  + \beta_{31}U_{31i}+ \beta_{32}U_{32i}+ \beta_{33}U_{33i}+ \beta_{34}U_{34i}+ \beta_{35}U_{35i} \\
& \quad \quad  +\beta_4X_{4i}+ \beta_{51}(U_{31i}\cdot X_{4i})+ \beta_{42}(U_{32i}\cdot X_{4i})+ \beta_{53}(U_{33i}\cdot X_{4i})+ \beta_{54}(U_{34i}\cdot X_{4i})+ \beta_{55}(U_{35i}\cdot X_{4i}) \\
& \quad \quad  +e_i
\end{align*}
where $e_i \overset{iid}{\sim} N(0,\sigma^2)$ and we are using our notation established for the main effects full model.

In @tbl-summary(a), we see that while our initial ANOVA tests suggested statistically significant parameters, many of the individual categorical parameters are not significant. However, we will continue with this model and note its limitations.
```{r}
#| label: tbl-summary
#| layout-nrow: 2
#| tbl-cap: "Final Model Summaries"
#| tbl-subcap: 
#|    - "Coefficient Estimates"
#|    - "Model Fit"

# Final model summary statistics.
kable(tidy(intMod.new)[,c(1:3,5)], digits = 4,
...
## Appendix
```{r remove-outliers-plots1, fig.height = 3.5}
#| layout-ncol: 2
#| layout-nrow: 2
#| label: fig-model-diagnostics-outlier
#| fig-cap: "Model Diagnostics Outliers Removed"
#| fig-subcap: 
#|    - "Residuals vs. Fitted"
#|    - "Residuals vs. Date"
#|    - "Residuals vs. Heat Index"
...
# Review residual diagnostics and model diagnostics.
...
```{r remove-outliers-plots2, fig.height = 1.5}
#| layout-nrow: 3
#| label: fig-residual-diagnostics-outlier
#| fig-cap: "Residual Diagnostics Outliers Removed"
#| fig-subcap: 
#|    - ""
#|    - ""
#|    - ""

# Outlier removed interaction residual diagnostics.
base + geom_point(aes(x = as.numeric(case), y = .hat)) +
  geom_hline(data = tibble(threshold = c("2p/n", "3p/n"), 
                           ref = c(2*17/dim(results.new)[1], 3*17/dim(results)[1])), 
             aes(yintercept = ref, color = threshold)) +
  labs(color = "Threshold", x = "Case ID", y = "Leverage")
base + geom_point(aes(x = as.numeric(case), y = .cooksd)) +
  labs(x = "Case ID", y = "Cook's Distance")
base + geom_point(aes(x = as.numeric(case), y = .ti)) + 
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_hline(yintercept = qt(1 - (0.05/(2*dim(results.new)[1])), df = 629 - 1), 
             linetype = "dotted") +
  geom_hline(yintercept = qt((0.05/(2*dim(results.new)[1])), df = 629 - 1), 
             linetype = "dotted") + 
  ylim(-5,5) +
  labs(x = "Case ID", y = "Studentized Residuals")
