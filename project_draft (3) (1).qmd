...
ggplot(data = results, aes()) + geom_boxplot(aes(x = proximity, y = percent_standing_avg))
...
```{r fig.height = 4}
## reduced model
reducedMod <- lm(percent_standing_avg ~ sex + competition + distance + rel_hum + wind_speed + heat_index + proximity, data = results)
...
intMod.new <- lm(percent_standing_avg ~ competition + sex + distance*heat_index, data = results.new)
...
Key significant parameter coefficients
Interaction of Distance with Heat Index: The interaction term suggests that for each unit increase in the heat index, the impact of running a 20k on mean percent deviation average is increased by about 0.345 units as compared to the baseline distance (10k). This interaction is significant and indicates that higher heat index values accompanied with longer race distances conditions associated with higher mean percent deviation average or generally longer mean finishing time with respect to the standing records.

Distance 20k (distance20k): Competing in a 20k race, as opposed to the baseline race distance, is associated with a significant decrease in the percent standing average score by about 7.817 percentage unit, after adjusting for other variables and setting heat_index as 0. 

Sex - Women (sexWomen): Being female is associated with an increase in the mean percent deviation average time by approximately 0.709 percentage units compared to males. This suggests higher mean percent deviation average or generally longer mean finishing time with respect to the male runners.

World Cup (competitionWorld Cup): Compared to the baseline category (Common Wealth competition), participating in the World Cup is associated with an increase in the mean percent standing average score by approximately 1.434 units.
```{r}
summary(intMod.new)
```
...
One limitation to the model is the lack of runners time in each race. Since the we only have the time for top 3 athletes, which are highly skilled and thus less affected by the weather and distances where as having more runner times will help us see the full scope of weather effect on runners performance through the mean deviation time. This is also the reason which lead to the low explanatory power of the model or R-squared. 

Some possible alterations could be using spline regression, generalized linear models or nonparametric regression to deal with the violation of normality assumption.
```{r}
...
ggplot(data = results, aes(x = heat_index, y = percent_standing_avg, color = distance)) + 
  geom_point()
```
