...
ggpairs(data = results[, c("air_temp", "dew_point", "wind_speed", "heat_index", 
                         "simp_wet_bulb", "rel_hum", "percent_standing_avg")]) +
...
cat.counts = as.data.frame(table(results$distance, 
                                 results$competition, 
                                 results$sex))
cat.counts = cat.counts %>% pivot_wider(names_from = c("Var1", "Var3"), 
                                        values_from = "Freq")
cat.counts$Var2 = c("Common Wealth", "Diamond League", "Gold Marathon",
                    "Olympic", "World Champ.", "World Cup")

table = cat.counts %>% kable(col.names = c("Comp.", "10K", "20K", "3K", "50K", 
                         "5K", "Mar.", "10K", "20K", "3K", "50K", "5K", "Mar."), 
                         booktabs = FALSE) %>% 
...
forwardMod <- stepAIC(object = nullMod, scope = list(upper = fullMod), 
                      direction = "forward", k = 2, trace = FALSE)
backwardMod <- stepAIC(object = fullMod, direction = "backward", 
                       k = 2, trace = FALSE)
# summary(stepMod)
# summary(forwardMod)
# summary(backwardMod)
...
intMod <- lm(percent_standing_avg ~ sex + competition + distance + heat_index + 
               distance:heat_index, data = results)
